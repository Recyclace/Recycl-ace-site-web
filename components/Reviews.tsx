...
function Stars({ n, size = 16 }: { n: number; size?: number }) {
  return (
    <span className="inline-flex" aria-label={`${n}/5`}>
      {[1, 2, 3, 4, 5].map((i) => (
        <svg key={i} width={size} height={size} viewBox="0 0 24 24" fill={i <= n ? "#F2B705" : "none"} stroke="#F2B705" strokeWidth="1.5">
              ...
              <div className="flex items-center gap-1">
                {[1, 2, 3, 4, 5].map((i) => (
                  <button key={i} type="button" onMouseEnter={() => setHover(i)} onMouseLeave={() => setHover(0)} onClick={() => setRating(i)} aria-label={`${i}/5`}>
                    <svg width="28" height="28" viewBox="0 0 24 24" fill={i <= (hover || rating) ? "#F2B705" : "none"} stroke="#F2B705" strokeWidth="1.5"><path d="M12 2l2.4 7.4H22l-6 4.4 2.3 7.2L12 16.6 5.7 21l2.3-7.2-6-4.4h7.6z" /></svg>
                  </button>
                ))}
              </div>
