...
// Icônes distinctes et parlantes
const IC = {
...
function Icon({ d, fill }: { d: string; fill?: boolean }) {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill={fill ? "#0F6B5B" : "none"} stroke="#0F6B5B" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
      <path d={d} />
    </svg>
  );
}
                  ...
                  <ul className="mt-6 space-y-4">
                    {s.feats.map((f, i) => (
                      <li key={i} className="flex items-center gap-3">
                        <span className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-foret/8">{<Icon d={f.icon} fill={f.fill} />}</span>
