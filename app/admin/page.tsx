...
type Order = { id: string; created_at: string; first_name: string; last_name: string; email: string; phone: string; city: string; country: string; items: any; total: number; status: string };
...
export default function AdminPage() {
  ...
  const exportOrders = () => { const rows = [["Date", "Nom", "Prénom", "Email", "Téléphone", "Ville", "Pays", "Total", "Statut"], ...orders.map((o) => [new Date(o.created_at).toLocaleDateString(), o.last_name, o.first_name, o.email, o.phone, o.city, o.country, o.total, o.status])]; const csv = rows.map((r) => r.map((v) => `"${String(v ?? "").replace(/"/g, '""')}"`).join(";")).join("\n"); const blob = new Blob(["﻿" + csv], { type: "text/csv" }); const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "commandes.csv"; a.click(); };
  ...
  return (
    <div className="container-x py-12">
      ...
      {tab === "orders" && (<div>
        ...
        <div className="mb-4 flex justify-end"><button onClick={exportOrders} className="btn-outline">{en ? "Export CSV" : "Exporter (CSV)"}</button></div>
        <div className="card overflow-x-auto"><table className="w-full text-sm"><thead className="bg-foret/5 text-left text-encre/60"><tr><th className="px-3 py-3">Date</th><th className="px-3 py-3">Client</th><th className="px-3 py-3">{en ? "Items" : "Équipements"}</th><th className="px-3 py-3">Total</th><th className="px-3 py-3">{en ? "Status" : "Statut"}</th></tr></thead>
          <tbody>{orders.length === 0 && <tr><td colSpan={5} className="px-3 py-6 text-center text-encre/50">{en ? "No orders yet (they appear once Stripe payment is connected)." : "Aucune commande (elles apparaîtront une fois le paiement Stripe branché)."}</td></tr>}
            {orders.map((o) => (<tr key={o.id} className="border-t border-foret/5 align-top"><td className="px-3 py-3 text-encre/60">{new Date(o.created_at).toLocaleDateString()}</td><td className="px-3 py-3 text-encre">{o.first_name} {o.last_name}<br /><span className="text-xs text-encre/50">{o.email} · {o.phone}<br />{o.city} · {o.country}</span></td><td className="px-3 py-3 text-encre/70">{Array.isArray(o.items) ? o.items.map((it: any, i: number) => <div key={i}>{it.name} × {it.qty}</div>) : "—"}</td><td className="px-3 py-3 font-semibold text-encre">{Number(o.total).toFixed(2)} €</td><td className="px-3 py-3"><select value={o.status} onChange={(e) => setOrderStatus(o.id, e.target.value)} className="rounded-lg border border-foret/15 px-2 py-1 text-sm">{ORDER_STATUS.map((s) => <option key={s} value={s}>{stLabel(s)}</option>)}</select></td></tr>))}</tbody></table></div>
        <p className="mt-3 text-xs text-encre/50">{en ? "Invoice (PDF) generation from selected orders will arrive with the Stripe payment module." : "La génération de factures (PDF) à partir des commandes sélectionnées arrivera avec le module de paiement Stripe."}</p>
      </div>)}
