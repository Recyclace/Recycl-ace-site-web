        ...
        <div className="container-x grid items-center gap-8 md:grid-cols-2">
          <div>
            ...
            <p className="mt-4 text-sable/80">
              {en
                ? "Two main materials: an outer felt, its seam, and a rubber core. Each of these can be recovered and given a second life — that's the heart of our process."
                : "Deux matériaux principaux : la feutrine, son joint, et un noyau en caoutchouc. Chacun peut être valorisé et retrouver une seconde vie — c'est tout l'enjeu de notre procédé."}
            </p>
          </div>
          <div className="relative">
            <Image src="/assets/savoir/anatomie-balle.svg" alt={en ? "Tennis ball anatomy" : "Anatomie d'une balle de tennis"} width={860} height={420} className="h-auto w-full" />
          </div>
        </div>
      </section>

      {/* 4 étapes — photos au même format */}
      <section className="container-x py-16 md:py-20">
        <div className="mb-12 text-center">
          <span className="eyebrow">{en ? "Our process" : "Notre processus"}</span>
          <h2 className="h-display mt-3 text-3xl text-encre md:text-4xl">{en ? "From the court to a second life" : "Du court à la seconde vie"}</h2>
        </div>
